function [img,H] = iradonn(varargin)
    ...
    center = floor((N + 1)/2);      %确定中心位置
    xleft = -center + 1;            %确定左边位置
    x = (1:N) - 1 + xleft;          %确定x的取值范围
    x = repmat(x, N, 1);            %将x向下排列N次
                                    %N = 2*floor(size(R,1)/(2*sqrt(2)))
    ytop = center - 1;              %确定y最高顶点位置
    y = (N:-1:1).' - N + ytop;      %将y划归到-n~n
    y = repmat(y, 1, N);            %y横向排列n次

    len = size(p,1);                %拍照长度
    ctrIdx = ceil(len/2);           %拍照中心
    ...
    switch interp
        ...
        case {'spline','pchip','cubic','v5cubic'}
            ...
            for i=1:length(theta)
                proj = p(:,i);                                          %p的第i列所有值512
                taxis = (1:size(p,1)) - ctrIdx;                         %t的axis为-1/2拍照中心~1/2拍照中心
                t = x.*costheta(i) + y.*sintheta(i);                    %对t做旋转也就是确定光照方向 坐标系方向
